...
- **`DEF-STATUS-CONNTYPE-ENUM`** — `franklinwh-cli status` reported a gateway connected over WiFi as `Ethernet 1`. `runtimeData.connType` (`0=4G, 1=WiFi, 2=Ethernet`) was being rendered through `NETWORK_TYPES`, which uses the unrelated `currentNetType` encoding. Added `const.devices.CONN_TYPE_NAMES` for this field and used it.
