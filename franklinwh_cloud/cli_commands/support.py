...
_WRAPPER_PATH = os.path.join(os.path.expanduser("~/.franklinwh"), "nettest-runner.sh")


def _generate_wrapper(bms: bool = False, fem_url: str | None = None) -> str:
    """Generate a shell wrapper that activates the venv and runs nettest.

    Modelled on the user's update.sh — auto-discovers and activates venv.
    Also embeds the absolute path to franklinwh.ini so the wrapper works
    under launchd/cron where CWD context is lost.
    """
    cli_path = _find_cli_path()
    # Derive venv bin dir from the CLI path (e.g. .../venv/bin/franklinwh-cli)
    cli_dir = os.path.dirname(os.path.abspath(cli_path))
    venv_activate = os.path.join(cli_dir, "activate")

    # Auto-discover config file (same search order as CLI)
    config_path = None
    for candidate in ["franklinwh.ini", "franklinwh/franklinwh.ini"]:
    ...
    cmd += f' support --nettest --record "{log_file}"'
    if bms:
        cmd += " --bms"
    if fem_url:
        cmd += f" --fem-url {fem_url}"

    config_note = f"#   Config: {config_path}" if config_path else "#   Config: (none found — will use env vars)"

    return f"""#!/bin/bash
# Auto-generated by franklinwh-cli --schedule
# Activates the Python venv and runs nettest
{config_note}
set -e
...
def _generate_plist(interval_s: int) -> str:
    """Generate macOS launchd plist XML — calls the wrapper script."""
...
def _generate_cron_entry(interval_s: int) -> str:
    """Generate a crontab line for Linux — calls the wrapper script."""
    if interval_s == 86400:
        ...
        cron = f"*/{mins} * * * *"
    return f"{cron} /bin/bash {_WRAPPER_PATH}  # {_SCHED_LABEL}"
...
def _schedule_create(interval_s: int, bms: bool, fem_url: str | None, is_mac: bool):
    ...
    # Generate and save the wrapper script
    wrapper = _generate_wrapper(bms=bms, fem_url=fem_url)
    ...
    if is_mac:
        plist = _generate_plist(interval_s)
        ...
        entry = _generate_cron_entry(interval_s)
...
def _schedule_once(bms: bool, fem_url: str | None, is_mac: bool):
    ...
    subprocess.Popen(["/bin/bash", _WRAPPER_PATH],
