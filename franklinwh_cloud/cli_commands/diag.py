...
async def run(client, *, json_output: bool = False):
    ...
    t0 = time.monotonic()
    try:
        # get_home_gateway_list is the lightest authenticated call
    ...
    try:
        ...
        # Phase detection from Cloud API
        is_three_phase = solar_vo.get("isThreePhaseInstall", False)
        if is_three_phase:
            phase = "Three Phase (L1/L2/L3)"
        else:
            phase = "Single Phase (L1)"
            # Note: Cloud API doesn't distinguish single vs split-phase
            # Only Modbus ACType register can tell split-phase (US 240V)

        # Gateway firmware/model from earlier gateway list
        gw_detail = {}
        for g in gateways:
