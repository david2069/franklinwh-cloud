...
class StatsMixin:
    ...
    async def get_stats(self, *, include_electrical: bool = False) -> Stats:
        ...
        # effective_mode: the top-level operating mode label — what the system is DOING.
        #
        # History: a previous version incorrectly used tou_mode_desc as effective_mode, causing
        # the UI and support snapshot to show the utility programme name instead of the mode.
        _VPP_PROGRAMME_ID = 9
        _mode_id = int(runtimedata_v2.get("mode", 0) or 0)
        if run_status == _VPP_PROGRAMME_ID or _mode_id == _VPP_PROGRAMME_ID:
            effective_mode = RUN_STATUS.get(_VPP_PROGRAMME_ID, "VPP Mode")
        else:
