...
|----|------|-------------|----------|
| DEF-BLANK-STATS-PASSTHROUGH | `mixins/stats.py` / `models.py` | When `get_device_composite_info()` returns HTTP 200 with `result: null` (cloud API glitch — distinct from a network timeout), `get_stats()` falls through to `empty_stats()` and returns all-zero telemetry: `battery_soc=0`, `solar=0`, `grid_connection_state=CONNECTED`. `StaleDataCache` only intercepts `TimeoutException` and `ConnectError` — it does not intercept a valid-HTTP-but-empty-payload response. Downstream consumers (FHAI, MQTT) then publish the zeros to Home Assistant, causing false 0% SoC, 0 kW solar, and no grid alert. Fix: add last-known-good `Stats` caching inside `get_stats()` — if `empty_stats()` would be returned, return the last valid `Stats` instead, tagged with a `is_stale: bool` flag on the object, and log a warning. Never publish zeros when a prior valid reading exists. | 2026-04-10 |
