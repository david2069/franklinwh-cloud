...
|----|------|-------------|----------|
| DEF-GRID-STATE-ENUM | Models / Mixins / CLI / FHAI | `grid_outage: bool` (introduced 2026-04-10) only captures two states — `Connected` and `Outage` — conflating `Not-Grid-Tied` and `Simulated-Off-Grid` both into `False`. The correct model is a `GridConnectionState` enum with four unambiguous values: `CONNECTED` (grid-tied, contactor closed), `OUTAGE` (firmware-detected grid loss), `NOT_GRID_TIED` (permanent island, `gridFlag=False` from `get_entrance_info()`), `SIMULATED_OFF_GRID` (user-selected off-grid mode, `selectOffgrid=True` from `get_grid_status()`). Fix: replace `grid_outage: bool` with `grid_connection_state: GridConnectionState` in `Current`. Enum values are unambiguous for every topology. `get_stats()` populates `CONNECTED`/`OUTAGE` from composite info; `NOT_GRID_TIED` and `SIMULATED_OFF_GRID` require `get_entrance_info()` and `get_grid_status()` respectively — available via `discover()` path or `get_stats(include_electrical=True)` extension. Breaking change — requires explicit authorization. | 2026-04-10 |
