...
|----|------|-------------|----------|
| DEF-GRID-STATE-ENUM | Models / Mixins / CLI / FHAI | `grid_outage: bool` (introduced 2026-04-10) only captures two states — `Connected` and `Outage` — conflating `Not-Grid-Tied` and `Simulated-Off-Grid` both into `False`. The correct model is a `GridConnectionState` enum with four values: `CONNECTED`, `OUTAGE`, `NOT_GRID_TIED`, `SIMULATED_OFF_GRID`. **Data source strategy (confirmed):** (1) `NOT_GRID_TIED` — determined once at startup from `get_entrance_info()` (`gridFlag=False`); cached in client/snapshot for the lifetime of the power-up cycle — zero per-poll cost; a non-grid-tied site will never become grid-tied. (2) `SIMULATED_OFF_GRID` — gated on `main_sw[0]=1` (raw firmware OPEN, i.e. grid relay disconnected) AND NOT an outage; only then call `get_grid_status()` to check `selectOffgrid=True`. On a normally-connected system `main_sw[0]=0` (CLOSED) so `get_grid_status()` is never called on the hot path. (3) `OUTAGE` — `offGridFlag=1` or `offgridreason>0` from composite info, already in `get_stats()`. (4) `CONNECTED` — all other cases. Fix: add `GridConnectionState` enum to `models.py`; replace `grid_outage: bool` with `grid_connection_state: GridConnectionState` in `Current`; populate in `get_stats()` using startup-cached `gridFlag` + `main_sw[0]` gate; `get_grid_status()` call only when relay is OPEN. Breaking change to `Current` — requires explicit authorization. | 2026-04-10 |
