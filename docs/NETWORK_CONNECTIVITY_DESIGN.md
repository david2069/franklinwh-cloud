...
| G2 | ~~**Two incompatible network enums.**~~ **CORRECTED 2026-08-30.** `commSetPara.currentNetType`, the extended 339 payload **and** `runtimeData.connType` all use the *same* encoding: `1=Eth0, 2=Eth1, 3=WiFi, 4=4G` (`NETWORK_TYPES`). The claim that `connType` used `0=4G, 1=WiFi, 2=Ethernet` originated in an unsourced `models.py` comment and is contradicted by **20,471 runtimeData samples**, in which `connType` is only ever `{2: 559, 3: 19797, 4: 115}` — 0 and 1 never occur, and 3 dominating matches a gateway that lives on WiFi. **The real hazard is the opposite one:** acting on the comment produced a regression that rendered every healthy gateway as `Unknown (3)`. What remains true is that a *bitmask-style* table (`3=WiFi+Ethernet`) belongs to neither field — see G2a. | Use `NETWORK_TYPES` for both. |
| G2a | Local bitmask-style tables (`0=None, 1=WiFi, 2=Ethernet, 3=WiFi+Ethernet, …`) existed in `diag.py` and `support.py` and matched **no** field either module read. | Rendered WiFi as "WiFi+Ethernet" and Ethernet 1 as "WiFi". Both deleted. |
